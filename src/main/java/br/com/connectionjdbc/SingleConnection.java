...
public class SingleConnection {
    ...
    private static String userHome = System.getProperty("user.home"); //Pega a pasta do usuário que executa o programa no sistema
    private static String url = "jdbc:sqlite:" + userHome + "\\database.db"; //Estabelece o url para a conexão da base de dados
    ...
    private static void connect() {
        try {
            if (connection == null) {
                Class.forName("org.sqlite.JDBC"); //driver JDBC

                connection = DriverManager.getConnection(url);
                connection.setAutoCommit(false);

                //Cria tb_categoria se não existir 
